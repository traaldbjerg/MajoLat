...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[scale=0.9]
        ...
        \node [inner sep=1pt, label=210:$p \vee q_1$] at (PVQ1) {};
    ...
    \end{tikzpicture}
    \caption{Geometric representation of the volume $E^+(p \parallel q_1, q_2)$ (shaded in red) for 3 states $p, q_1, q_2 \in \mathcal{P}^d$. The analogy with Figure \ref{fig:venn_three} is quite clear: we add back $H(p \vee q_1 \vee q_2)$ because it is removed twice in the formula.}
...
If one could rigorously prove that an entropy-based set function would give a true measure on the $\sigma$-algebra of majorization cones, then one would immediately have a new proof of supermodularity of Shannon entropy. This is because any measure $\mu$ is monotone (similiar to resource monotonicity), in the sense that for any 2 sets $A, B$, if $A \subseteq B$, then $\mu(A) \leq \mu(B)$. Consider now figure \ref{fig:volumic_supermodularity}.
