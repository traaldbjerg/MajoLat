...
\begin{equation}
    H(\lambda^{AB}) \geq H(\lambda^A) \quad \text{and} \quad H(\lambda^{AB}) \geq H(\lambda^B). \label{eq:old_criterion}
\end{equation}
...
This new formulation in terms of entropies is a strengthening of the initial entropic criterion (\ref{eq:old_criterion}), providing a better lower bound on the entropy of a separable state. However, it is still not as strong as the majorization criterion itself. Moreover, this new entropic criterion based on the meet is not very interesting for pure bipartite states because of Corollary \ref{cor:reduced_schmidt}, which essentially states that $\lambda^A = \lambda^B$ if $\rho^{AB}$ is pure. In that case $\lambda^A \wedge \lambda^B = \lambda^A = \lambda^B$, and the new entropic criterion reduces to the original one. While pure bipartite states are the more interesting states for quantum computation, as decoherence (i.e. losing purity by interacting with the environment) is never a desired feature for qubits as mixed states introduce uncertainty into computations, in practice noise always makes states at least slightly mixed. Figure \ref{fig:improved_criterion} illustrates the new criterion.
...
\noindent where we use $\beta(p, q)$ instead of the true join because the smoothing algorithm is an additional layer of complexity. The minima and maxima of those expressions seem difficult to realize with unitaries.
...
\subsection{Simulations for R\'enyi entropies} \label{sec:renyi_simulation}
...
This problem unfortunately prevents us from achieving a monotone obeying Eq. (\ref{eq:ideal_incomparability}), at least directly. However, we can define two separate notions of incomparability, one as the minimal distance to the future cone of $q$ and the other as the minimal distance to the past cone of $q$.
...
\begin{figure}[h!]
    ...
    \end{tikzpicture}
    \caption{Depiction of the definition of the future incomparability function, $E^+(p \parallel q)$, which is the entropic distance from $p$ to the closest point of the future cone of $q$. Visually, we expect that point to be the join.}
...
\begin{definition}[Future incomparability function] \label{def:future_incomparability_monotone}
    ...
    \begin{equation}
        E^+ (p \parallel q) = \min_{s \: | \: s \succ q} d(p, s).
    \end{equation}
\end{definition}
...
\begin{proof}
    ...
    \end{align}
    \noindent Let $s'[s] = p \vee s$. The absorption law $a \vee a = a$, along with the associativity of the join operation, implies that choosing $s = s'[s]$ (i.e. requiring that $s$ majorizes $p$) makes the second term of the sum vanish, while the first remains invariant.
    \begin{align}
        \implies E^+ (p \parallel q) &= \min_{s \succ q} d(p, p \vee s'[s]) + d(p \vee s'[s], s'[s]) \\
        &= \min_{s \: | \: s \succ q} d(p, p \vee (p \vee s)) + d(p \vee (p \vee s), p \vee s) \\
        &= \min_{s \: | \: s \succ q} d(p, (p \vee p) \vee s) + d((p \vee p) \vee s, p \vee s)\\
        &= \min_{s \: | \: s \succ q} d(p, p \vee s) + d(p \vee s, p \vee s)\\
        &= \min_{s \: | \: s \succ q} d(p, p \vee s)
    \end{align}
    \noindent and so for any choice of $s$, taking $p \vee s$ instead will always give a smaller distance. Because $s$ majorizes $q$, $s'[s]$ majorizes $q$ too. Moreover, to be the join of $p$ with another vector, $s'[s]$ must majorize $p$ as well, and so we can write
    \begin{align}
        E^+ (p \parallel q) &= \min_{s' \: | \: s' \succ q, p} d(p, s') \\
        &= \min_{s' \: | \: s' \succ q, p} H(p) + H(s') - 2H(p \vee s') \\
        &= \min_{s' \: | \: s' \succ q, p} H(p) + H(s') - 2H(s') \\
        &= \min_{s' \: | \: s' \succ q, p} H(p) - H(s').
    \end{align}
    \noindent The Schur-concavity of the Shannon entropy implies that the maximum value of $H(s')$ for $s' \in \{v \mid v \succ p, q\}$\footnote{This is equivalent to saying $s' \in \mathcal{T}_+(p) \cap \mathcal{T}_+(q)$.}$ \subseteq \mathcal{P}^d$ is reached for a vector $s'$ that is majorized by all other vectors in the subset. By definition, that vector is the join $p \vee q$. Thus, $E^+(p \parallel q) = H(p) - H(p \vee q)$. \qedhere
\end{proof}
...
\begin{definition}[Past incomparability function]
    ...
    \begin{equation}
        E^- (p \parallel q) = \min_{s \: | \: s \prec q} d'(p, s).
    \end{equation}
\end{definition}
...
\begin{proof}
    ...
    \begin{align}
        E^- (p \parallel q) &= \min_{s \: | \: s \prec q} d'(p, s) \\
        \overset{\text{Lemma \ref{lem:comp_past}}}&{=} \min_{s \: | \: s \prec q} d'(p, p \wedge s) + d'(p \wedge s, s) \\ 
        &\implies \min_{s \: | \: s \prec q} d'(p, s) \geq \min_{s \: | \: s \prec q} d'(p, p \wedge s) \label{eq:1}
    \end{align}
    \noindent Let $s'[s] = p \wedge s$. Because $s$ is majorized by $q$, $s'[s]$ is majorized by $q$ as well. Moreover, to be the meet of $p$ with another vector, $s'[s]$ must be majorized by $p$ as well.
    \begin{align}
        \implies \min_{s \: | \: s \prec q} d'(p, p \wedge s) &= \min_{s' \: | \: s' \prec q, p} d'(p, s')\\
        &= \min_{s' \: | \: s' \prec q, p} 2H(p \wedge s') - H(p) - H(s')\\
        &= \min_{s' \: | \: s' \prec q, p} H(s') - H(p).
    \end{align}
...
\begin{figure}[h!] % rajouter les 4 distances sur la figure ?
    ...
    \end{tikzpicture}
    \caption{Depiction of the expected monotonicity in $p$ of $E^+$. The dotted red half-cone contains all possible bistochastic degradations of $p$. Visually, points on the edge closest to $q$, like $p'$, have the smallest distance to the future cone of $q$. Other choices of degradation, like $p''$, clearly increase the distance to the cone.}
    \label{fig:future_p_monotonicity}
\end{figure}

In order to rigorously prove monotonicity, we will first need the following lemma (cf. Figure \ref{fig:hyperbolic_geometry}).
...
\begin{theorem} \label{th:monotone_future_p}
    For all bistochastic matrices $D$, $E^+ (Dp \parallel q) \geq E^+ (p \parallel q)$.
\end{theorem}
...
\begin{proof}
    ...
    \begin{align}
        \min_{p' \: | \: p' \prec p} E^+ (p' \parallel q) \overset{\text{Theorem \ref{th:closest_future}}}&{=} \min_{p' \: | \: p' \prec p} d(p', p' \vee q)\\
        \overset{\text{Def. of $q'$}}&{=} \min_{p' \: | \: p' \prec p} d(p', q'[p'])\\
        \overset{\text{(\ref{eq:construction})}}&{=} \min_{p' \: | \: p' \prec p} d(p''[p'], q'[p'])\\
        \overset{\text{Def. of $p''$}}&{=} \min_{p' \: | \: p' \prec p} d(p \wedge q'[p'], q'[p'])\\
        \overset{\text{Lemma \ref{lem:incomparable_diamond}}}&{\geq} \min_{p' \: | \: p' \prec p} d(p, p \vee q'[p'])\\
        \overset{\text{Def. of $q'$}}&{=} \min_{p' \: | \: p' \prec p} d(p, p \vee (p' \vee q))\\
        &= \min_{p' \: | \: p' \prec p} d(p, (p \vee p') \vee q)\\
        &= d(p, p \vee q)\\
        &= E^+ (p \parallel q),
    \end{align}
...
This theorem is quite satisfying in the sense that it seems to indicate that our geometric intuitions on the lattice are valid. Moreover, we can now promote the future incomparability function to a future incomparability \textit{monotone}. Theorem \ref{th:monotone_future_p} has a few corollaries in the quantum regime.
...
\begin{figure}[h!] % rajouter les 4 distances sur la figure ?
    ...
    \begin{tikzpicture}[scale=0.8]
        % draw cone of q
        \coordinate (Q) at (0, 2);
        \coordinate (QL) at (-2,5);
        \coordinate (QR) at (2, 5);
        \draw [name path=Q--QL] (Q) -- (QL);
        \draw [name path=Q--QR] (Q) -- (QR);
        % drawfill future cone of q
        \fill[fill=blue, opacity=0.2] (Q) -- (QL) -- (QR) -- cycle;
        % draw cone of p
        \coordinate (P) at (-2, 1.5);
        \coordinate (PL) at (-4.333, 5);
        \coordinate (PR) at (0.333, 5);
        \draw[dotted] [name path=P--PL, color=red] (P) -- (PL);
        \draw[dotted] [name path=P--PR, color=red] (P) -- (PR);
        % draw top right leg for p
        \coordinate (PRT) at (0.333, 5);
        \draw[draw=none] [name path=P--PRT] (P) -- (PRT);
        \path [name intersections={of=P--PRT and Q--QL, by=L}];
        \draw[dotted] [name path=P--L, color=gray] (P) -- (L) node[midway, below, sloped] {$d(p, p \wedge q)$};
        % add p' and p''
        \coordinate (P') at (-3, 3);
        \coordinate (P'') at (-1.5, 3.5);
        % ghost draw top right cone leg for p' and p''
        \coordinate (P'RT) at (-1.666, 5);
        \coordinate (P''RT) at (-0.5, 5);
        \draw[draw=none] [name path=P'--P'RT] (P') -- (P'RT);
        \draw[draw=none] [name path=P''--P''RT] (P'') -- (P''RT);
        \path [name intersections={of=P'--P'RT and Q--QL, by=I}];
        \path [name intersections={of=P''--P''RT and Q--QL, by=J}];
        \draw[dotted] [name path=P'--I, color=gray] (P') -- (I) node[midway, above, sloped] {$d(p', p' \wedge q)$};
        \draw[dotted] [name path=P''--J, color=gray] (P'') -- (J);
        % draw nodes at the end to prevent dotted drawings on them
        \node [fill=black,inner sep=1pt,label=0:$q$] at (Q) {};
        \node [inner sep=0pt, label=90:$\mathcal{T}_- (q)$] at (0, 3.5) {};
        \node [fill=black,inner sep=1pt,label=225:$p$] at (P) {};
        \node [fill=black,inner sep=1pt,label=225:$p'$] at (P') {};
        \node [fill=black,inner sep=1pt,label=145:$p''$] at (P'') {};
    \end{tikzpicture}
    \caption{Depiction of why monotonicity in $p$ of $E^-$ fails. The dotted red half-cone contains all possible bistochastic degradations of $p$.  We can of course make $E^-$ decrease by going straight towards the past cone of $q$, but the issue arises when sticking to the left edge, as the hyperbolic geometry makes $E^-$ slightly increase. This explains why there is no monotonicity in $p$.}
    \label{fig:past_p_monotonicity_intuition}
\end{figure}
...
Let us now turn our attention to bistochastic degradations of $q$, and attempt to promote our incomparability functions to monotones. Figures \ref{fig:future_q_monotonicity} and \ref{fig:past_q_monotonicity} show the monotonicity relations we expect. The proofs of these properties are simpler than for probe degradation, and can be found in Appendix \ref{app:q_monotonicity}. Starting with the future incomparability function, if we can show that $E^+(p \parallel Dq) \leq E^+(p \parallel q)$ for any bistochastic matrix $D$, then $E^+$ would be a decreasing monotone, like we expect.
