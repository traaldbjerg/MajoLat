...
Lorenz curves were first introduced in 1905 by economist Max Lorenz as a way to represent income inequality. By plotting the cumulative sums of two distributions, we can then say that if one curve is above the other at all times, then it is more unequal (more disordered) than the second. By normalizing the distributions for total income, this graphical tool could then be used to quickly compare income inequality between different regions or countries.

More formally, the Lorenz curve of a vector $p \in \mathbb{R}^d$ is obtained by linear interpolation between the points of the set $\{(i, S^\downarrow_i(p)) | i \in \mathbb{N}, i \leq d\}$. % il y a d'office une notation moins moche pour ca
We will be using the convention that $S^\downarrow_0 (v) = 0$ for any vector $v$. %The alias $L^\downarrow_p (x) \coloneqq S^\downarrow_x (p)$ is an alternative notation used for Lorenz curves, where the abscissa goes in the argument of the function.
A few examples of Lorenz curves are given in figures \ref{fig:lorenz_comparable_example} and \ref{fig:lorenz_incomparable_example}.
...
\begin{figure}[h!]
    ...
    \begin{minipage}{.48\textwidth}
        ...
        \caption{Lorenz curve for an example with comparable vectors $p = (0.5, 0.4, 0.1)$, $q = (0.4, 0.4, 0.2)$.}
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Lorenz curve for the incomparable vector $p = (0.5, 0.25, 0.25)$, $q = (0.4, 0.4, 0.2)$ example.}
        \label{fig:lorenz_incomparable_example}
    \end{minipage}
\end{figure}

These figures are fairly easy to interpret: if a curve is above another at all points, then at each coordinate point its cumulative sum is larger than the cumulative sum of the other vector. At each integer abscissa we see one of the inequalities from definition \ref{def:majorization} ! From these figures, we quite quickly get the intuition that if one vector majorizes another (and so they are comparable), then all of the inequalities must be satisfied and the majorizer's curve must be above the majorized's curve at all points. If the two vectors are incomparable, then at least one inequality must fail, and so there must be a reversal in the largest cumulative sum, and the Lorenz curves must cross at least once.
...
A simple example of a lattice is the set $\mathbb{N}$ of natural numbers ordered by the relation “divides”. Then for $a, b \in \mathbb{N}$ the lub is l.c.m.$(a, b)$ and the glb is g.c.d.$(a, b)$, where l.c.m. = least common multiple and g.c.d. = greatest common divisor. % example from Dominich, 2008, Elements of Lattice Theory
...
\begin{figure}[h!]
    ...
    \caption{Representation of the $p = (0.6, 0.2, 0.2)$ and $q = (0.45, 0.45, 0.1)$ example, along with the prototype $\beta(p, q)$ glb vector.} \label{fig:join_naive_attempt}
\end{figure}
...
\begin{theorem}[Supermodularity of the Shannon entropy \cite{cicalese_supermodularity_2002}]
...
\begin{theorem}[Subadditivity of the Shannon entropy, \cite{cicalese_supermodularity_2002}]
...
\begin{definition}[Entropic distance]
