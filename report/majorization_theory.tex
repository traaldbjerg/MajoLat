...
\begin{align}
    d'(p, p \vee q) + d'(p \vee q, q) &= 2 H(p \wedge (p \vee q)) - H(p) - H(p \vee q)\nonumber \\
...
This quantity can be compared to $d'(p, q) = 2 H(p \wedge q) - H(p) - H(q)$, and we have $d'(p, q) \geq d'(p, p \vee q) + d'(p \vee q, q)$ iff
\begin{align}
