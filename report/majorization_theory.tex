...
\begin{proof} \label{proof:alternative_supermodularity}
    ...
    \end{gather}
    We have $\sum_{i=1}^{2d} A_i = \sum_{i=1}^{2d} B_i = 2$. We will now show that $A \succ B$ (which is actually stronger than the original theorem). Clearly, the $k^{\text{th}}$ cumulative sum of $A$ is made up of the biggest entries of $p$ and $q$, and the same goes for cumulative sums of $B$ being made of the biggest entries of $r$ and $s$. To avoid ill-defined terms, we additionally introduce the convention that $S^\downarrow_k (v) = 1 \; \forall k > d$ for any $d$-dimensional probability vector $v$. For all $k \leq 2d$, we have 
    \begin{gather}
