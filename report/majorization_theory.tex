...
\begin{definition}[Order-preserving concatenation]
    Let $p, q \in \mathcal{P}^d$, then the concatenation of $p$ and $q$, $\omega(p, q)$, is defined as
    \begin{align}
        \omega(p, q) = (p_1, p_2, ..., p_d, q_1, q_2, ..., q_d) \in \mathbb{R}^{2d}.
    \end{align}
    We then define the order-preserving concatenation of $p$ and $q$, $\gamma(p, q)$, which is constructed with the following algorithm. Let $k, l \in \mathbb{N}$, and initialize them to 1. $\forall i \leq 2d$,
    \begin{itemize}
        \item if $p_k \geq q_l$, then $\gamma_i = p_k$ and increment $k$ by 1
        \item if $p_k < q_l$, then $\gamma_i = q_l$ and increment $l$ by 1.
    \end{itemize}
    Note that in general, $\gamma(p, q) \neq \gamma(p, q)^\downarrow$.
\end{definition}

\begin{theorem}[Supermodularity of all "sum-concave" functions]
    All functions of the form
    \begin{equation} \label{eq:sum-concave}
        F: \mathcal{P}^n \rightarrow \mathbb{R},  p \rightarrow \sum_i \varphi(p_i), \
    \end{equation}
    where $\varphi$ is a real function that is concave on $[0, 1]$, are supermodular on the majorization lattice\footnote{It is important to specify which lattice: on the boolean lattice, which gives a different ordering and different meet and join operations, the Shannon entropy is \textit{submodular} instead.}, and so for all $p, q \in \mathcal{P}^d$,
...
\begin{proof} \label{proof:alternative_supermodularity}
    ...
    Recall equations (\ref{eq:alpha_bis}) and (\ref{eq:beta_bis}), from which we immediately deduce\footnote{The point of the order-preserving concatenation is to be able to write the following, if we had used $(r_1, r_2, \dots, r_d, s_1, s_2, \dots, s_d)^\downarrow$ instead of $\gamma(r, s)$ we would not be able to use the nice expressions of $S^\downarrow_k (r)$ and $S^\downarrow_k (s)$ in terms of $S^\downarrow_i (p)$ and $S^\downarrow_i (q)$.}
    \begin{gather}
...
\begin{corollary}[Supermodularity of Shannon entropy \cite{cicalese_supermodularity_2002}]
...
\begin{corollary}[Supermodularity of \textit{all} Rényi entropies]
    The Rényi entropy functions $H_\alpha$ are all supermodular on the majorization lattice, and so for all $p, q \in \mathcal{P}^d$,
    \begin{equation} \label{eq:supermodularity_renyi}
        H_\alpha(p \wedge q) + H_\alpha(p \vee q) \geq H_\alpha(p) + H_\alpha(q).
    \end{equation}
    Note that if $p \sim q$, there is trivially equality.
\end{corollary}

\noindent We have thus proven a powerful generalisation of the supermodularity property.
